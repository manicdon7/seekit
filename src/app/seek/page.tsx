...
const SeekPage: React.FC = () => {
  ...
  // Check if lostItems is an array before filtering
  ...
  const renderItems = (items: (FoundItem | LostItem)[]) => {
    return items.length > 0 ? (
      items.map((item) => (
        <div
          key={item._id}
          className="bg-gray-700 p-6 rounded-lg shadow-lg flex flex-col mb-4"
        >
          {item.imageURL && (
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-gray-900 to-gray-800 text-white font-anton">
      <Head>
        ...
        <meta
          name="description"
          content={`Explore ${showFoundItems ? "found" : "lost"
            } items shared by the community on Seekit.`}
        />
        <meta
          property="og:title"
      ...
      <div className="container mx-auto p-8">
        ...
        <div className="flex justify-center mb-6 bg-gray-800 p-5 rounded-full gap-10 shadow-md overflow-hidden">
          <button
            className={`flex-1 px-6 py-3 rounded-full text-2xl font-semibold ${showFoundItems ? "bg-[#4ECCA3]" : "bg-gray-600"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {showFoundItems ? renderItems(filteredFoundItems) : renderItems(filteredLostItems)}
        </div>
      </div>
